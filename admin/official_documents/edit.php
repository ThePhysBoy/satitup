...
// ดึงข้อมูลเอกสาร
$sql = "SELECT * FROM official_documents WHERE id = ?";
...
if (!$document) {
    $_SESSION['error'] = "ไม่พบเอกสารที่ต้องการแก้ไข";
    header("Location: index.php");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "UPDATE official_documents SET 
            doc_type = ?, doc_number = ?, category_id = ?, title = ?, 
            description = ?, file_path = ?, publisher_name = ?, 
            publisher_position = ?, publish_date = ?, effective_date = ?, 
            status = ? 
            WHERE id = ?";
    ...
    if ($stmt->execute()) {
        $_SESSION['success'] = "แก้ไขเอกสารสำเร็จ";
        header("Location: index.php");
        exit();
    } else {
...
<head>
    ...
    <title>แก้ไขเอกสาร | Admin Panel</title>
...
<body>
    ...
    <div class="main-container">
        <div class="page-header">
            <h1><i class="fas fa-edit me-2"></i> แก้ไขเอกสาร</h1>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item"><a href="index.php" class="text-white-50">จัดการเอกสาร</a></li>
                    <li class="breadcrumb-item active text-white" aria-current="page">แก้ไขเอกสาร</li>
                </ol>
            </nav>
        </div>
        ...
        <form method="POST" enctype="multipart/form-data" class="form-card">
            <div class="form-section">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select" id="doc_type" name="doc_type" required onchange="updateCategories()">
                            ...
                            <option value="regulation" <?php echo $document['doc_type'] == 'regulation' ? 'selected' : ''; ?>>ข้อบังคับ</option>
                            <option value="rule" <?php echo $document['doc_type'] == 'rule' ? 'selected' : ''; ?>>ระเบียบ</option>
                            <option value="announcement" <?php echo $document['doc_type'] == 'announcement' ? 'selected' : ''; ?>>ประกาศ</option>
                            <option value="order" <?php echo $document['doc_type'] == 'order' ? 'selected' : ''; ?>>คำสั่ง</option>
                        </select>
                    </div>
                ...
                <div class="row">
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="text" class="form-control" id="doc_number" name="doc_number" 
                               value="<?php echo htmlspecialchars($document['doc_number']); ?>" placeholder="เช่น 1/2567">
                    </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="description" name="description" rows="4"><?php echo htmlspecialchars($document['description']); ?></textarea>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <small class="form-text text-muted">อัพโหลดไฟล์ใหม่เพื่อแทนที่ไฟล์เดิม (รองรับเฉพาะ PDF ขนาดไม่เกิน 10MB)</small>
                </div>
            </div>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control" id="publisher_name" name="publisher_name" 
                               value="<?php echo htmlspecialchars($document['publisher_name']); ?>" required>
                    </div>
                    
                    <div class="col-md-6 mb-3">
                        <label for="publisher_position" class="form-label required">ตำแหน่ง</label>
                        <input type="text" class="form-control" id="publisher_position" name="publisher_position" 
                               value="<?php echo htmlspecialchars($document['publisher_position']); ?>" required>
                    </div>
                </div>
            </div>
                ...
                <div class="row">
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="date" class="form-control" id="publish_date" name="publish_date" 
                               value="<?php echo $document['publish_date']; ?>" required>
                    </div>
                    
                    <div class="col-md-4 mb-3">
                        <label for="effective_date" class="form-label">วันที่มีผลบังคับใช้</label>
                        <input type="date" class="form-control" id="effective_date" name="effective_date" 
                               value="<?php echo $document['effective_date']; ?>">
                    </div>
                    
                    <div class="col-md-4 mb-3">
                        <label for="status" class="form-label required">สถานะ</label>
                        <select class="form-select" id="status" name="status" required>
                            <option value="active" <?php echo $document['status'] == 'active' ? 'selected' : ''; ?>>เผยแพร่</option>
                            <option value="draft" <?php echo $document['status'] == 'draft' ? 'selected' : ''; ?>>ฉบับร่าง</option>
                            <option value="inactive" <?php echo $document['status'] == 'inactive' ? 'selected' : ''; ?>>ไม่เผยแพร่</option>
                        </select>
                    </div>
                </div>
            </div>
    ...
    const currentCategoryId = <?php echo $document['category_id'] ?: 'null'; ?>;
