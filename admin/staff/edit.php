...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $image_path = $staff['image_path']; // Keep existing image by default
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            $stmt = $conn->prepare("DELETE FROM staff_positions WHERE staff_id = ?");
            ...
            header("Location: index.php?success=แก้ไขข้อมูลบุคลากรเรียบร้อยแล้ว");
            exit;
            ...
            // Delete uploaded image if exists and is new
            if (!empty($image_path) && $image_path !== $staff['image_path'] && file_exists('../../' . $image_path)) {
                unlink('../../' . $image_path);
            }
        }
    }
...
// Set page title
$page_title = "แก้ไขข้อมูลบุคลากร";
$include_summernote = true;

// Set template variables
$page_header_icon = '<i class="fas fa-user-edit me-3"></i>';
$back_button = true;
$back_url = 'index.php';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let positionIndex = <?php echo count($positions); ?>;
...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0 text-gray-800">แก้ไขข้อมูลบุคลากร</h1>
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">แก้ไขข้อมูลบุคลากร</h6>
        </div>
        <div class="card-body">
                                        ...
                                        <div id="positions-container">
                                            <?php foreach ($positions as $index => $position): ?>
                                                <div class="position-item mb-2 d-flex align-items-center">
                                                    <input type="text" class="form-control me-2" name="position_name[]" placeholder="ชื่อตำแหน่ง" required value="<?php echo htmlspecialchars($position['position_name']); ?>">
                                                    <div class="form-check me-2">
                                                        <input class="form-check-input primary-position" type="checkbox" name="is_primary_position[<?php echo $index; ?>]" id="is_primary_position_<?php echo $index; ?>" <?php echo $position['is_primary'] ? 'checked' : ''; ?>>
                                                        <label class="form-check-label" for="is_primary_position_<?php echo $index; ?>">หลัก</label>
                                                    </div>
                                                    <button type="button" class="btn btn-sm btn-danger remove-position <?php echo count($positions) <= 1 ? 'd-none' : ''; ?>">
                                                        <i class="fas fa-times"></i>
                                                    </button>
                                                </div>
                                            <?php endforeach; ?>
                                    ...
                                    <div class="col-sm-9 offset-sm-3">
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" id="is_head" name="is_head" 
                                                   <?php echo $staff['is_head'] ? 'checked' : ''; ?>>
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-9">
                                        <input type="number" class="form-control" id="order_number" name="order_number" min="0" 
                                               value="<?php echo (int)$staff['order_number']; ?>">
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-9">
                                        <select class="form-select" id="status" name="status">
                                            ...
                                            <option value="inactive" <?php echo $staff['status'] === 'inactive' ? 'selected' : ''; ?>>ปิดใช้งาน</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-10">
                                        <textarea class="form-control summernote" id="bio" name="bio"><?php echo htmlspecialchars($staff['bio'] ?? ''); ?></textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
