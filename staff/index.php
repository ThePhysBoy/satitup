...
$support_departments = getDepartments('support', $conn);
$primary_departments = getDepartments('primary', $conn);

// Get current department ID from URL
$current_department_id = isset($_GET['department']) ? (int)$_GET['department'] : 0;

// Get department type from URL or default to 'academic'
$current_type = isset($_GET['type']) ? $_GET['type'] : 'academic';
if ($current_type !== 'academic' && $current_type !== 'support' && $current_type !== 'primary') {
    $current_type = 'academic';
}
...
if ($current_department_id > 0) {
    ...
    $staff_data = getStaffByType($current_type, $conn);
}
...
$page_title = "บุคลากร" . $type_labels[$current_type];
                ...
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">บุคลากร</li>
                        <li class="breadcrumb-item active" aria-current="page">
                            <?php echo $type_labels[$current_type]; ?>
                        </li>
            ...
            <div class="card shadow-sm">
                <div class="card-header bg-primary text-white">
                    ...
                    <div class="list-group list-group-flush">
                        <a href="?type=academic" class="list-group-item list-group-item-action <?php echo $current_type === 'academic' ? 'active' : ''; ?>">
                            <i class="fas fa-chalkboard-teacher me-2"></i> บุคลากรสายวิชาการ
                        </a>
                        <a href="?type=primary" class="list-group-item list-group-item-action <?php echo $current_type === 'primary' ? 'active' : ''; ?>">
                            <i class="fas fa-child me-2"></i> บุคลากรประถมศึกษา
                        </a>
                        <a href="?type=support" class="list-group-item list-group-item-action <?php echo $current_type === 'support' ? 'active' : ''; ?>">
                            <i class="fas fa-user-cog me-2"></i> บุคลากรสายสนับสนุน
                        </a>
                    </div>
                
                <?php if ($current_type === 'academic' && !empty($academic_departments)): ?>
                    <div class="card-header bg-light">
                        <h6 class="mb-0">กลุ่มสาระการเรียนรู้</h6>
                    </div>
                    <div class="list-group list-group-flush">
                        <a href="?type=academic" class="list-group-item list-group-item-action <?php echo $current_type === 'academic' && $current_department_id === 0 ? 'active' : ''; ?>">
                            <i class="fas fa-users me-2"></i> ทุกกลุ่มสาระ
                        </a>
                        <?php foreach ($academic_departments as $dept): ?>
                            <a href="?type=academic&department=<?php echo $dept['id']; ?>" 
                ...
                <?php endif; ?>
                    ...
                    <?php if ($current_type === 'primary' && !empty($primary_departments)): ?>
                        <div class="card-header bg-light">
                            <h6 class="mb-0">ชั้นประถมศึกษา</h6>
                        </div>
                        <div class="list-group list-group-flush">
                            <a href="?type=primary" class="list-group-item list-group-item-action <?php echo $current_type === 'primary' && $current_department_id === 0 ? 'active' : ''; ?>">
                                <i class="fas fa-users me-2"></i> ทุกชั้นเรียน
                            </a>
                            <?php foreach ($primary_departments as $dept): ?>
                                <a href="?type=primary&department=<?php echo $dept['id']; ?>" 
                                   class="list-group-item list-group-item-action <?php echo $current_department_id === (int)$dept['id'] ? 'active' : ''; ?>">
                                    <i class="fas fa-angle-right me-2"></i> <?php echo htmlspecialchars($dept['name']); ?>
                                </a>
                            <?php endforeach; ?>
                    ...
                    <?php if ($current_type === 'support' && !empty($support_departments)): ?>
                        <div class="card-header bg-light">
                            <h6 class="mb-0">หน่วยงานสายสนับสนุน</h6>
                        </div>
                        <div class="list-group list-group-flush">
                            <a href="?type=support" class="list-group-item list-group-item-action <?php echo $current_type === 'support' && $current_department_id === 0 ? 'active' : ''; ?>">
                                <i class="fas fa-users me-2"></i> ทุกหน่วยงาน
                            </a>
                            <?php foreach ($support_departments as $dept): ?>
                                <a href="?type=support&department=<?php echo $dept['id']; ?>" 
                                   class="list-group-item list-group-item-action <?php echo $current_department_id === (int)$dept['id'] ? 'active' : ''; ?>">
                                    <i class="fas fa-angle-right me-2"></i> <?php echo htmlspecialchars($dept['name']); ?>
                                </a>
                            <?php endforeach; ?>
